...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/login", "/register", "/saveUser", "/static/**", "/css/**", "/error").permitAll()
                        .requestMatchers("/reportDetails").hasRole("IDEATOR") // Access restricted to IDEATOR role
                        .requestMatchers("/admin/**").hasRole("ADMIN") // Protect /admin/** for ADMIN role
                        .requestMatchers("/ideator/**").hasRole("IDEATOR")
                        .requestMatchers("/investor/**").hasRole("INVESTOR")
                        .requestMatchers("/projects/view/**").hasAnyRole("ADMIN", "IDEATOR", "INVESTOR") // Allow access to projects for these roles
                        .anyRequest().authenticated()
                )
                .formLogin(form -> form
                        .loginPage("/login")
